...
MODEL_VERSION = "v0.1 defensive_fund_ml_85_15"
...
def update_paper_trading() -> dict:
    ...
    candidates = _read_named_csv(f"defensive_fund_ml_85_15_{latest_date}_named.csv")
...
def _latest_signal_date() -> str:
    files = sorted(SIGNAL_DIR.glob("defensive_fund_ml_85_15_*.csv"))
    if not files:
        raise FileNotFoundError("No defensive signal file found. Run scripts/run_today_model_signals.py first.")
    dates = [
        path.stem.replace("defensive_fund_ml_85_15_", "").replace("_named", "")
        for path in files
    ]
